...
const getMonthDetails = (year, month) => {
    let firstDay = new Date(year, month, 1).getDay(); // Ayın ilk gününün haftanın hangi günü olduğunu belirler
    let numberOfDays = getNumberOfDays(year, month);
    let monthArray = [];
    let index = 0;
  
    // Ayın tüm günleri için detayları hesapla
    for (let i = 0; i < numberOfDays; i++) {
      let dayDetails = getDayDetails({
        index: i + firstDay,
        numberOfDays,
        firstDay,
        year,
...
const setCalBody = (monthDetails) => {
    // İlk günün haftanın hangi günü olduğunu belirle
    const firstDayOfWeek = monthDetails[0].day;
  
    // Takvim başına boş hücreler ekle
    for (let i = 0; i < firstDayOfWeek; i++) {
      let emptyDiv = document.createElement("div");
      emptyDiv.classList.add("cell_wrapper");
      calendar.appendChild(emptyDiv);
    }
  
    // Mevcut ayın günlerini ekle ve tıklama işlevi ata
    for (let i = 0; i < monthDetails.length; i++) {
      ...
      if (monthDetails[i].month === 0) {
        ...
        isCurrentDay(monthDetails[i], div); // Bugünü işaretle
      }
      span.classList.add("cell_item");
      span.innerText = monthDetails[i].date;
  
      // Gün seçme işlevi
      div.addEventListener("click", () => selectDay(monthDetails[i], div));
